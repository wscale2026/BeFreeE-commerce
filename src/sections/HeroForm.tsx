...
import { ArrowUp, ArrowDown, Check, ChevronRight, Lock, CreditCard, ShieldCheck } from "lucide-react";
...
export default function HeroForm() {
  ...
  const handleSubmit = () => {
    // Redirection to payment / member space
    alert("Redirection vers l'espace de paiement sécurisé Stripe...");
  };
      ...
      <div className="section-container">
              ...
              {!showSummary ? (
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col">
                        <textarea
                          ...
                          rows={4}
                          className="w-full bg-transparent border-b-2 border-gray-200 focus:border-brand-blue outline-none text-lg py-3 px-1 resize-none transition-colors"
                        />
                ...
                >
                  <div className="text-center mb-6">
                    ...
                    <p className="text-gray-500">Basée sur vos réponses, voici votre diagnostic</p>
                  </div>

                  {/* Modules */}
                  <div className="bg-brand-gray rounded-xl p-5 mb-6 space-y-3">
                    ...
                    {getModules().map((mod) => (
                      <div key={mod} className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-brand-dark font-medium">{mod}</span>
                      </div>
                    ))}
                  </div>

                  {/* Price */}
                  <div className="text-center mb-6 p-4 border-2 border-brand-blue rounded-xl bg-blue-50">
                    <p className="text-sm text-gray-600 mb-1">Investissement unique et forfaitaire</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                    <button
                      ...
                      className="flex items-center justify-center gap-2 bg-brand-blue text-white font-semibold py-3.5 px-4 rounded-xl hover:bg-blue-700 transition-all"
                    >
                    ...
                    <button
                      onClick={handleSubmit}
                      className="flex items-center justify-center gap-2 bg-white border-2 border-brand-blue text-brand-blue font-semibold py-3.5 px-4 rounded-xl hover:bg-blue-50 transition-all"
                    >
                  ...
                  <div className="flex justify-end gap-2 mt-6">
                    <button
                      onClick={handlePrev}
                      className="w-10 h-10 rounded-full flex items-center justify-center border border-gray-300 text-gray-600 hover:border-brand-blue hover:text-brand-blue transition-all"
                    >
                      <ArrowUp className="w-4 h-4" />
                    </button>
                  </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-10 md:mb-14">
          {["Agence en ligne", "Formateur", "Coach", "Consultant"].map((tag) => (
            <span
              key={tag}
              className="px-5 py-2.5 bg-white border border-gray-200 rounded-full text-sm text-gray-600 font-medium shadow-sm hover:shadow-md hover:border-brand-blue/30 hover:text-brand-blue transition-all duration-300 cursor-default"
            >
              {tag}
            </span>
          ))}
